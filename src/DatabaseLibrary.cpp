...
// Their name should consist with number and/or letters, and the underscore sharacters
...
// Get all the basic Element Information from the Database
bool ElementDatabaseArray::FindGetAllElementInfo(wxString GetElement, wxString GetGammaPeak, wxChoice* &ListElement, wxChoice* &ListGammaPeak, wxTextCtrl* &SetNumber, wxTextCtrl* &SetAbundance, wxTextCtrl* &SetMass, wxTextCtrl* &SetIsotopic)
{
  // Clear old tables
  SetNumber->Clear();
  SetAbundance->Clear();
...
ZieglerParameters::ZieglerParameters( wxChoice* choiceZieglerVersion, wxGrid* tableZieglerParameters, wxString &info)
{
   ...
   wxString temp14 = tableZieglerParameters->GetCellValue(i,14);
   wxString temp15 = tableZieglerParameters->GetCellValue(i,15);
...
// For compatibility with ERYA-Bulk
void ZieglerParameters::AddZieglerParameterData(wxString e, wxString v1, wxString v2, wxString v3, wxString v4, wxString v5, wxString v6, wxString v7, wxString v8, wxString v9, wxString v10, wxString v11, wxString v12, wxString m)
{
 ...
 ZieglerMass.Add(m);
 ZieglerDensity.Add(wxT("1")); //dummy value
 ZieglerBloch.Add(wxT("10")); //default value
 return;
}

// Add a new element for the Ziegler Parameters
void ZieglerParameters::AddZieglerParameterData(wxString e, wxString v1, wxString v2, wxString v3, wxString v4, wxString v5, wxString v6, wxString v7, wxString v8, wxString v9, wxString v10, wxString v11, wxString v12, wxString m, wxString d, wxString b)
{
 ZieglerElements.Add(e);
 ZieglerValues1.Add(v1);
 ZieglerValues2.Add(v2);
  ...
  for(int i=0;i<ZieglerElements.GetCount();i++)
   {
    ...
    ZieglerTable->SetCellValue(i,14,ZieglerDensity.Item(i));
    ZieglerTable->SetCellValue(i,15,ZieglerBloch.Item(i));
   }
  return true;
}
...
bool ZieglerParameters::FixDensity(wxArrayString ListDensity)
{
 for(int k=0; k < ZieglerElements.GetCount(); k++)
 {
  if(k<ListDensity.GetCount())
  {
  double temp;
  if(ListDensity.Item(k).ToDouble(&temp))
   ZieglerDensity.Item(k) = wxString::Format("%.8f",temp);
  else
   ZieglerDensity.Item(k) = wxString::Format("%.8f",1.0);
  }
 }
 return true;
}

bool ZieglerParameters::FixBloch(wxArrayString ListBloch)
{
 for(int k=0; k < ZieglerElements.GetCount(); k++)
 {
  if(k<ListBloch.GetCount())
  {
  double temp;
  if(ListBloch.Item(k).ToDouble(&temp))
   ZieglerBloch.Item(k) = wxString::Format("%.8f",temp);
  else
   ZieglerBloch.Item(k) = wxString::Format("%.8f",10.0);
  }
 }
 return true;
}
