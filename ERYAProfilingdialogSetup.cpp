/***************************************************************
 * Name:      ERYAProfilingdialogSetup.cpp
...
void ERYAProfilingdialogSetup::OnDefaultSave( wxCommandEvent& event )
{
 wxMessageDialog *dial = new wxMessageDialog(NULL, wxT("Do you want to save the changed settings?"), wxT("Save Edited Selection"), wxYES_NO | wxNO_DEFAULT | wxICON_WARNING);
  if (dial->ShowModal() == wxID_YES)
  {
    ...
    // Call the parent frame
   ERYAProfilingERYAProfilingMain *Parent = (ERYAProfilingERYAProfilingMain *) GetParent();
    if(ThisDatabase.ERYAProfilingDatabaseFileLoad())
    {
   ...
   if(Parent->ReplaceSetupFile())
   {
     ...
     {
     wxMessageDialog *dial = new wxMessageDialog(NULL, wxT("Cannot save a new ERYA Profiling configuration file.\nRefer to the User Guide for details."), wxT("Saving Error!"), wxOK | wxICON_ERROR);
     dial->ShowModal();
     Parent->ProgramQuit(false);
     Close();
     }
   ...
   {
   wxMessageDialog *dial = new wxMessageDialog(NULL, wxT("Cannot create a new ERYA Profiling configuration file.\nRefer to the User Guide for details."), wxT("Reset Error!"), wxOK | wxICON_ERROR);
   dial->ShowModal();
   Parent->ProgramQuit(false);
   Close();
   }
...
void ERYAProfilingdialogSetup::OnDefaultReset( wxCommandEvent& event )
{
...
void ERYAProfilingdialogSetup::OnDefaultDelete( wxCommandEvent& event )
{
...
// Quit any edition, without saving it.
void ERYAProfilingdialogSetup::OnDefaultQuit( wxCommandEvent& event )
{
 // Set flag
     ERYAProfilingERYAProfilingMain *Parent = (ERYAProfilingERYAProfilingMain *) GetParent();
     Parent->ProgramQuit(false);
